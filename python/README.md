...
This is a FastAPI + SQLite service that ingests Stripe-like webhook events and updates orders. You will implement the webhook endpoint and ensure it handles edge cases correctly.
...
- The database schema includes a unique constraint: `UniqueConstraint("provider", "event_id")`
- Consider what happens when you try to insert a duplicate
...
- Use FastAPI `BackgroundTasks` to return 200 immediately
- Process the event (update order) in the background
...
The database includes these test orders (created via `seed.py` or manually):

```sql
INSERT INTO orders (id, status, updated_at)
VALUES
  ('11111111-1111-1111-1111-111111111111', 'pending', CURRENT_TIMESTAMP),
  ('22222222-2222-2222-2222-222222222222', 'paid', CURRENT_TIMESTAMP),
  ('33333333-3333-3333-3333-333333333333', 'pending', CURRENT_TIMESTAMP);
```
...
├── webhooks.py          # TODO: Your implementation here
├── orders.py            # Helper: mark_order_paid(db, order_id)
├── models.py            # SQLAlchemy models (Order, WebhookEvent)
├── schemas.py           # Pydantic schemas (StripeEvent, WebhookResponse)
├── db.py                # Database setup
└── tests/
    └── test_webhook.py  # TODO: Add/update tests
```
...
- ✅ Handles duplicate inserts gracefully (e.g., `IntegrityError`)
...
- ❌ Over-engineered solution (Kafka, complex abstractions for this exercise)
...
- Check `app/orders.py` for the `mark_order_paid()` helper function
- Look at `app/models.py` to see the database models
- The `WebhookResponse` schema has fields you can use: `ok`, `duplicate`, `processed`, `error`
